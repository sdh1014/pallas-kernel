...
class TestFusedRMSNormGated(unittest.TestCase):
    ...
    def test_triton_vs_cpu(self):
        ...
        with torch.no_grad():
            ...
            y_cpu = cpu_fn(x, g)
        assert compare_tensor("FusedRMSNormGated", y_triton, y_cpu, atol=1e-5, rtol=1e-5)
...
class TestShortConvolution(unittest.TestCase):
    ...
    def test_triton_vs_cpu(self):
        ...
        with torch.no_grad():
            ...
            y_cpu, _ = cpu_conv(x)
        assert compare_tensor("ShortConv output", y_triton.cpu(), y_cpu, atol=1e-5, rtol=1e-5)
    ...
    def test_triton_vs_cpu_cache(self):
        ...
        assert compare_tensor("cache", cache_triton.cpu(), cache_cpu, atol=1e-5, rtol=1e-5)
    ...
    def test_causality(self):
        ...
        y1, _ = conv(x1)
        y2, _ = conv(x2)
        assert torch.allclose(y1[:, :10], y2[:, :10], atol=1e-6), "past outputs affected by future"
        assert not torch.allclose(y1[:, 10:], y2[:, 10:], atol=1e-6), "future should differ"
