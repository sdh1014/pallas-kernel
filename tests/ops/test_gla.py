...
class TestChunkGLA(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        assert compare_tensor(
            "final_state", s_triton.cpu(), s_cpu, atol=2e-2, rtol=2e-2
        )
...
class TestFusedRecurrentGLA(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        assert compare_tensor(
            "final_state", s_triton.cpu(), s_cpu, atol=1e-4, rtol=1e-4
        )
    ...
    def test_vs_naive(self):
        ...
        o_fused, ht_fused = cpu_fused_recurrent_gla(
            q, k, v, gk=gk, initial_state=h0, output_final_state=True
        )
        assert compare_tensor(
...
class TestChunkSubFunctions(unittest.TestCase):
    ...
    def test_chunk_fwd_h(self):
        ...
        assert compare_tensor(
            "final state", chunk_ht, naive_ht.float(), atol=1e-4, rtol=1e-4
        )
        NT = T // C
    ...
    def test_chunk_gla_fwd_intra_gk(self):
        ...
        A = cpu_chunk_gla_fwd_intra_gk(
            q.float(), k.float(), g_cumsum, scale, chunk_size=C
        )
