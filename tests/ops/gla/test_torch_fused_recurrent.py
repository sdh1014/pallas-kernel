...
def _run_gold(q, k, v, *, gk=None, gv=None, h0=None, cu=None,
...
def _run_cpu(q, k, v, *, gk=None, gv=None, h0=None, cu=None,
             scale=None, reverse=False):
...
def test_gold_vs_cpu(cfg):
    ...
    o_gold, s_gold = _run_gold(q, k, v, gk=gk, gv=gv, h0=h0, cu=cu,
                                scale=scale, reverse=reverse)
    o_cpu, s_cpu = _run_cpu(q, k, v, gk=gk, gv=gv, h0=h0, cu=cu,
                             scale=scale, reverse=reverse)
...
def test_state_split():
    ...
    assert compare_tensor("full state (gold vs cpu)", s_full_gold, s_full_cpu, atol=1e-4, rtol=1e-4)
    assert compare_tensor("gold: full vs split", s_full_gold, s2_gold, atol=1e-4, rtol=1e-4)
    assert compare_tensor("cpu: full vs split", s_full_cpu, s2_cpu, atol=1e-5, rtol=1e-5)
    assert compare_tensor("split state (gold vs cpu)", s2_gold, s2_cpu, atol=1e-4, rtol=1e-4)
...
def test_varlen_packed_vs_separate():
    ...
    q1, k1, v1 = torch.randn(1, s1_len, H, K), torch.randn(1, s1_len, H, K), torch.randn(1, s1_len, H, V)
    g1 = F.logsigmoid(torch.randn(1, s1_len, H, K))
    q2, k2, v2 = torch.randn(1, s2_len, H, K), torch.randn(1, s2_len, H, K), torch.randn(1, s2_len, H, V)
    ...
    assert compare_tensor("seg1 output", o1_gold, o_cpu[:, :s1_len], atol=1e-4, rtol=1e-4)
    assert compare_tensor("seg2 output", o2_gold, o_cpu[:, s1_len:], atol=1e-4, rtol=1e-4)
    assert compare_tensor("seg1 state", s1_gold.squeeze(0), s_cpu[0], atol=1e-4, rtol=1e-4)
    assert compare_tensor("seg2 state", s2_gold.squeeze(0), s_cpu[1], atol=1e-4, rtol=1e-4)
...
def test_no_final_state():
    ...
    o_gold, s_gold = triton_fused_recurrent(
        *_to_device(q, k, v), gk=gk.to(DEVICE), output_final_state=False,
    )
