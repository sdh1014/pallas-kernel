...
try:
    from fla.ops.gla.chunk import chunk_gla_fwd_intra_gk as triton_chunk_gla_fwd_intra_gk
...
def test_gold_vs_cpu(cfg):
    ...
    assert compare_tensor("A_intra", A_triton_masked, A_cpu_masked, atol=atol, rtol=rtol)
...
def test_gold_varlen_vs_segments(cfg):
    ...
    A_triton = triton_chunk_gla_fwd_intra_gk(
        *_to_device(q_packed, k_packed, g_packed),
        scale=scale, cu_seqlens=cu_seqlens.to(DEVICE), chunk_size=C,
    ).cpu()
    ...
    assert compare_tensor("A_intra_varlen", A_triton_masked, A_ref_masked, atol=atol, rtol=rtol)
