...
def test_pallas_vs_cpu(cfg):
    ...
    # CPU reference
    o_cpu = cpu_chunk_gla_fwd_o_gk(q, v, g, A_cpu, h, scale=scale,
                                    chunk_size=chunk_size)

    # Pallas TPU
    o_pallas = pallas_chunk_gla_fwd_o_gk(
        _torch_to_jax(q), _torch_to_jax(v), _torch_to_jax(g),
        _torch_to_jax(A_pallas), _torch_to_jax(h),
...
def _make_segment_inputs(B, L, H, K, V, chunk_size, scale):
    ...
    o_ref_full = cpu_chunk_gla_fwd_o_gk(q_p, v_p, g_p, A_seg, h_seg,
...
def test_pallas_varlen_vs_segments(cfg):
    ...
    for L in seqlens:
        q_raw, v_raw, g_raw, A_seg, h_seg, o_ref = \
            _make_segment_inputs(B, L, H, K, V, C, scale)
        qs.append(q_raw)
        vs.append(v_raw)
        gs.append(g_raw)
        A_all.append(A_seg)
    ...
    # Run Pallas
    o_pallas = pallas_chunk_gla_fwd_o_gk(
        _torch_to_jax(q_packed), _torch_to_jax(v_packed),
        _torch_to_jax(g_packed), _torch_to_jax(A_packed),
