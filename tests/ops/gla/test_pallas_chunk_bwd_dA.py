"""chunk_gla_bwd_dA: JAX ref vs Torch CPU reference tests."""
...
def test_chunk_gla_bwd_dA(cfg):
    ...
    dA_jax = jax_chunk_gla_bwd_dA(v_j, do_j, scale, chunk_size=C)

    assert compare_tensor("dA", dA_cpu, dA_jax, atol=1e-5, rtol=1e-5)
