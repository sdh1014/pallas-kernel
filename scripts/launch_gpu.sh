...
TEMP_YAML="${SCRIPT_DIR}/gpu_resource_rendered.sky.yaml"
...
sed -e "s|\$ACCELERATOR|${ACCELERATOR}|g" \
    -e "s|\$NAME|${NAME}|g" \
    "${SCRIPT_DIR}/gpu_resource.sky.yaml" > "$TEMP_YAML"

# Create cluster name with ref
CLUSTER_NAME="gpu-$SANITIZED_NAME-$RANDOM"

# Save cluster name to file for other scripts to use
echo "$CLUSTER_NAME" > "${SCRIPT_DIR}/../.cluster_name_gpu"
