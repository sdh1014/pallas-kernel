    ...
    *,
    ...
    for i_b in range(N):
        ...
        for i_t in range(t):
            ...
            g_b = jnp.exp(g_b)
            kv = jnp.einsum("hk,hv->hkv", k_b, v_b, precision=jax.lax.Precision.HIGHEST, preferred_element_type=jnp.float32)  # [H, K, V]
            s = jnp.add(jnp.multiply(s, g_b[..., None]), kv)
            o_b = jnp.einsum("hk,hkv->hv", q_b, s, precision=jax.lax.Precision.HIGHEST, preferred_element_type=jnp.float32)  # [H, V]
